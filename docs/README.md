...
|--------|------:|------:|-------------|
| Graphics | 73 | 19,900 | DirectX 11 렌더링 (Forward/Deferred, PBR, Shadow, PostProcess, IBL, SSAO, SSR, TAA, SSGI, 20 서브시스템) |
| Assets | 18 | 4,544 | 정적/스켈레탈 메시, 스켈레톤, 애니메이션, 모델 로더 (FBX/OBJ/GLTF), KLightComponent |
| UI | 27 | 2,183 | 캔버스 기반 UI (Text, Button, Image, Slider, Checkbox, Layouts) |
| Physics | 6 | 937 | 리지드바디, 충돌 감지, Raycast |
| Serialization | 4 | 973 | 바이너리/JSON 아카이브 |
| Core | 3 | 935 | KEngine - 윈도우 관리, 메인 루프, 서브시스템 소유, ISubsystem 인터페이스, KSubsystemRegistry |
| Audio | 6 | 910 | XAudio2 오디오, 3D 사운드 |
| Input | 3 | 655 | 키보드, 마우스, Raw Input, Action Mapping |
| Scene | 4 | 659 | Actor-Component 시스템, 씬 관리 |
...
|-----------|------:|------:|-------------|
| EngineInterop | 2 | 1,143 | C++ DLL (flat C API, 107 exported functions) for C# P/Invoke |
